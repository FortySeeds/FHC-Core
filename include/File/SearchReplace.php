...
class File_SearchReplace
{
    ...
    function pregSearch($filename)
    {
        ...
        $file       = fread($fp = fopen($filename, 'r'), filesize($filename)); fclose($fp);
        $occurences = numberOfElements($matches = preg_split($this->find, $file)) - 1;
    ...
    {
        if ($this->find != '') {
            if ((is_array($this->files) AND numberOfElements($this->files) > 0) OR $this->files != '') $this->doFiles($this->search_function);
